...
async def test_send_reminders_calls_dingtalk(mem_session):
    ...
    with patch("scheduler.DingTalkClient") as mock_cls,          patch("scheduler.Session", _make_session_mock(mem_session)):
        mock_dt = AsyncMock()
...
async def test_expire_old_packages(mem_session):
    ...
    mem_session.expire(old_pkg); mem_session.refresh(old_pkg)
...
async def test_pending_not_expired(mem_session):
    ...
    mem_session.expire(recent_pkg); mem_session.refresh(recent_pkg)
